...
async def auto_load_files():
    """Auto-load latest expense JSON and CSV statements from default directories."""
    try:
        ...
        uploaded_files = []
        for csv_path in STATEMENTS_DIR.glob("*.csv"):
...
async def process_files(
    ...
    """Upload existing JSON and CSV/Excel files for bank-config selection."""
    try:
...
async def auto_categorize(bank_selections: List[dict]):
    """Process uploaded files with the selected bank configs and auto-categorize."""
    try:
...
async def export_json():
    """Export the merged existing + newly-categorized transactions as a JSONL download."""
    try:
        existing_data = processing_state["existing_data"]
        new_transactions = processing_state["new_transactions"]

        merged_df = pd.DataFrame(existing_data + new_transactions)
...
async def get_categories():
    ...
    try:
        ...
        return {"categories": list(processor.category_mapping.keys())}
